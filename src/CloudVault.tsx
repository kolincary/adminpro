...
const CloudVault: React.FC<CloudVaultProps> = ({ user, isAdmin, isDevMode }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // We need two readers or one after another. 
    // Let's use one reader for ArrayBuffer to get preview data and then convert to base64.
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Convert original file to base64 for "as-is" storage using a more robust method
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-2xl flex items-center justify-center shadow-xl shadow-emerald-900/20 border border-white/10">
            <Cloud className="w-8 h-8 text-white" />
          </div>
          <div>
            <h3 className="text-2xl font-black text-white tracking-tight text-shadow-sm">Cloud Vault</h3>
            <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-1">Transfer Data Antar Perangkat</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <label className={`flex items-center gap-2 px-6 py-3 ${uploading ? 'bg-slate-700' : 'bg-indigo-600 hover:bg-indigo-500'} text-white font-black rounded-2xl transition-all shadow-xl shadow-indigo-900/20 text-[10px] uppercase tracking-widest cursor-pointer`}>
      ...
      <div className="glass-card p-8 rounded-[40px] border-white/5">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari file di vault..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-6 py-4 bg-[#0f172a] border border-white/10 rounded-2xl text-white placeholder:text-slate-600 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            />
          </div>
          <div 
          ...
          >
            <input
              ...
              className="w-full pl-12 pr-6 py-4 bg-[#0f172a] border border-white/10 rounded-2xl text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all appearance-none cursor-pointer [color-scheme:dark] text-center"
            />
            ...
            {dateFilter && (
              <button 
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-rose-400 uppercase tracking-widest hover:text-rose-300 z-10"
              >
                Reset
              </button>
            )}
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center gap-4">
            ...
            <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Menghubungkan ke Vault...</p>
          </div>
        ) : filteredItems.length === 0 ? (
          <div className="py-20 flex flex-col items-center gap-6 opacity-20">
            <FileUp className="w-20 h-20 text-slate-500" />
            <div className="text-center">
              <p className="text-lg font-black uppercase tracking-widest text-slate-500">Vault Kosong</p>
              <p className="text-xs font-bold text-slate-600 mt-2">Unggah file Excel untuk membagikannya antar perangkat</p>
            </div>
          </div>
        ) : (
                  ...
                  <AnimatePresence>
                    {groupedItems[dateKey].map((item) => (
                      <motion.div
                        ...
                        className="group relative bg-[#0f172a] border border-white/5 rounded-[32px] p-6 hover:border-indigo-500/30 transition-all hover:shadow-2xl hover:shadow-indigo-500/10"
                      >
                        ...
                        <h4 className="text-white font-black text-sm truncate mb-2 pr-4" title={item.fileName}>
                          {item.fileName}
                        </h4>
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-[#0f172a] border border-white/10 rounded-[40px] p-8 shadow-2xl overflow-hidden"
            >
              ...
              <div className="flex flex-col items-center text-center gap-6">
                ...
                <div className="flex items-center gap-3 w-full">
                  <button
                    onClick={() => setDeleteConfirm(null)}
                    className="flex-1 px-6 py-4 bg-white/5 hover:bg-white/10 text-white font-black rounded-2xl transition-all text-[10px] uppercase tracking-widest border border-white/5"
                  >
                    Batal
                  </button>
                  <button
                    onClick={deleteItem}
                    className="flex-1 px-6 py-4 bg-rose-600 hover:bg-rose-500 text-white font-black rounded-2xl transition-all shadow-xl shadow-rose-900/40 text-[10px] uppercase tracking-widest"
                  >
                    Hapus Permanen
                  </button>
                </div>
            ...
            >
              ...
              <div className="flex flex-col items-center text-center gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed">
                    Tindakan ini akan menghapus <span className="text-white font-bold">SELURUH</span> file yang ada di Cloud Vault secara permanen dari server. Apakah Anda yakin?
                  </p>
                </div>
                <div className="flex items-center gap-3 w-full">
                  <button
